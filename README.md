...
高コストなクラウドAIを避け「Edge AI」を採用することで推論コストをゼロ化し、画像をエッジ側で圧縮することで通信コストを極小化しています。さらにCloud Storageには「一定期間で画像をArchiveクラス（超低価格層）へ自動階層化する」ライフサイクル管理を強制適用。これにより、GCPの「Always Free（無料枠）」の範囲内で安全にPoCを開始しつつ、エンタープライズに求められる数年間の監査証跡保存義務を満たす設計としています。
